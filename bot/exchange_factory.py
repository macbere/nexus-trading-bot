...
def build_exchange(config=None):
    """
    Returns config dict as the 'exchange' object.
    All API calls use direct REST - no ccxt needed.
    """
    global _cfg_cache
    if config is None:
    ...
    return config  # config IS the exchange object now
...
def fetch_ohlcv_direct(symbol, timeframe="1m", limit=100):
    """
    Fetch OHLCV candles via direct REST API.
    ...
    try:
        ...
        granularity = granularity_map.get(timeframe, "1min")
        url = (
